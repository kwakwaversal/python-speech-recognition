...
from auditok.cmdline import file_to_audio_source
import sys

try:

   asource = file_to_audio_source(filename='harvard.wav')
   # We set the `record` argument to True so that we can rewind the source
   # asource = ADSFactory.ads(filename='harvard.wav', record=True)
   ...
   tokenizer = StreamTokenizer(validator=validator, min_length=20, max_length=500, max_continuous_silence=30, mode=StreamTokenizer.DROP_TRAILING_SILENCE)
